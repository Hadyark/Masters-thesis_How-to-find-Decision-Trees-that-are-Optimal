...
import graphviz
import ast
from error_cython import discr_add2
...
def train_test_split(random_state, X, y, sensitive):
    ...
    y_train = y.drop(index=index_test).to_numpy(dtype='int')
    sensitive_train = sensitive.drop(index=index_test).to_numpy(dtype='int')

    X_test = X.drop(index=index_train).to_numpy()
    y_test = list(y.drop(index=index_train).to_numpy(dtype='int'))
    sensitive_test = sensitive.drop(index=index_train).to_numpy(dtype='int')
...
def error(tids, k, y, sensitive):
    ...
    summ = sum(supports)
    if len(classes) == 2:
        error0 = summ - supports[0]
        error1 = summ - supports[1] + abs(discr_add2(tids, y, sensitive)) * k
    elif len(classes) == 1:
        if classes[0] == 0:
            error0 = summ - supports[0]
            error1 = summ + abs(discr_add2(tids, y, sensitive)) * k
        elif classes[0] == 1:
            error0 = summ
            error1 = summ - supports[0] + abs(discr_add2(tids, y, sensitive)) * k
